...
namespace EmmerceAPIHCMUE.Models
{
  public class User
  {
    ...
    public bool SignUp()
    {
      string query = "insert into dbo.users(idUser, Email,PasswordHash, firstName, lastName, birthday, phoneNumber, address, note,province, interestedIn,lastLogin,avatar,AccessFailedCount,EmailConfirmed,PhoneNumberConfirmed,TwoFactorEnabled,LockoutEnabled) values('" + Helper.Instance.GenarateID() + "', '" + this.email + "', '" + Helper.Instance.HashPassword(this.password) + "', N'" + this.firstName + "', N'" + this.lastName + "', '" + this.birthday + "', '" + this.phoneNumber + "', N'" + this.address + "', '" + this.note + "', '" + this.province + "', '" + this.interestedIn + "', '" + this.lastLogin + "', '" + this.avatar + "',0,'True','True','False','False')";
    ...
    public bool UpdateUser()
    {
      ...
      {
        query += "PasswordHash = '" + this.password + "',";
      }
    ...
    public bool SignIn()
    {
      string query = "select PasswordHash from dbo.users where Email = '" + this.email + "'";
    ...
    {
      string queryCheck = "select PasswordHash from dbo.users where idUser = '" + this.idUser + "'";
      string pass = (string)Connection.Instance.ExecuteScalar(queryCheck);
      if (pass == null || pass == "")
      {
        return false;
      }
      bool compare = Helper.Instance.ValidatePassword(this.avatar, pass);
      if (!compare)
      {
        return false;
      }
      string query = "update dbo.users set PasswordHash = '" + Helper.Instance.HashPassword(this.password) + "' where idUser = '" + this.idUser + "'";
      int rowExec = Connection.Instance.ExecuteNonQuery(query);
      if (rowExec == 1)
      {
        return true;
    ...
    public bool ResetPassword(string newPassword)
    {
      ...
      string query = "update dbo.users set PasswordHash = '" + hashedPassword + "' where idUser = '" + id + "'";
